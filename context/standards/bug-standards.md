...
*   **Status:** The current state of the bug. The system shall use one of the following statuses:
    ...
    *   **[WONTFIX]:** The bug will not be fixed. This may be because it is obsolete, is working as intended, or requires a new feature that is not yet prioritised.
*   **Short Description:** A brief, clear summary of the bug. The system shall use the format: `[Observed Incorrect Behaviour] INSTEAD [Expected Correct Behaviour]` (e.g., "User is redirected to /home on logout INSTEAD user should be redirected to /login").
