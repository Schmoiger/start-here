...
SECRET_PATTERNS = [
    ...
    (re.compile(r"\b(?:sk-[a-zA-Z0-9]{32,})\b"), "OpenAI / API Secret Key"),
...
def scan_file_for_secrets(file_path: Path) -> list[str]:
    ...
    if file_path.name == ".env" or file_path.name.endswith(".env"):
        try:
            ...
            if any(re.search(pat[0], content) for pat in SECRET_PATTERNS) or "KEY=" in content or "SECRET=" in content:
                violations.append(
